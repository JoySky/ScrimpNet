...
namespace ScrimpNet.Core
{
    ...
    public partial class CoreConfig
    {
        ...
        public static class Log
        {
            ...
            /// {app}.Log.{ts}
            /// </summary>
            private const string FILE_PREFIX = "{app}.Log.{ts}";
            ...
            public static LoggerLevels LoggerLevels
            {
                get
                {
                    ...
                    _loggerLevels  = ConfigUtils.AppSetting<LoggerLevels>("ScrimpNet.Logging.LogLevels", LoggerLevels.Vital);
                    return _loggerLevels.Value;
                }
            }
            ...
            /// </summary>
            /// <remarks>
            /// Using native ConfigurationManager calls here since ConfigUtils...calls IsLastChangeLog
            /// </remarks>
            ...
            /// <summary>
            /// Folder where last chance log file will be stored
            /// </summary>
            internal static string LastChanceLogFolder
            {
                get
                {
                    string folderName = ConfigurationManager.AppSettings["ScrimpNet.Logging.LastChanceLogFolder"];
                    if (string.IsNullOrEmpty(folderName) == true ||Directory.Exists(folderName) == false )
                    {
                        folderName = "";
                    }
                   
                    return ReplaceTokens(folderName).Replace("{ts}", string.Format("{0:" + TimeStampFormat + "}", DateTime.Now));
                }
            }
            ...
            private static ILogPersister _activePersister;
            ...
            /// </remarks>
            public static ILogPersister ActivePersister
            {
                get
                {
                    ...
                    string persisterClassName = ConfigUtils.AppSetting<string>("ScrimpNet.Logging.Persister", "ScrimpNet.Core.Diagnostics.InternalLogFilePersister,ScrimpNet.Core");
           
                    try
                    {
                        _activePersister = ProviderFactory<ILogPersister>.GetInstance(persisterClassName);
                        if (_activePersister == null)
                        {
                            ScrimpNet.Core.Diagnostics.Log.LastChanceLog("Unable to create instance of {0} as ILogPersister", persisterClassName);
                            throw new ConfigurationErrorsException(TextUtils.StringFormat("Unable to create instance of {0} as ILogPersister", persisterClassName));
                        }
                    }
                    catch (Exception ex)
                    {
                        ScrimpNet.Core.Diagnostics.Log.LastChanceLog(ex, "Unable to create instance of {0} as ILogPersister", persisterClassName);
                        throw new ConfigurationErrorsException(TextUtils.StringFormat("Unable to create instance of {0} as ILogPersister", persisterClassName), ex);
                    }
            ...
            public static string LogFolder
            {
             get
             {
                 ...
                 _logFolder =  ReplaceTokens(ConfigUtils.AppSetting<string>("ScrimpNet.Logging.Default.LogFolder", Directory.GetCurrentDirectory()));
            ...
            public static string LogFilePrefix
            {
             get
             {
                 return CoreConfig.Log.ReplaceTokens(ConfigUtils.AppSetting<string>("ScrimpNet.Logging.Default.FilePrefix", FILE_PREFIX)); 
             }
            }
            ...
            public static int MaximumFileSizeMb
            {
                get
                {
                    return ConfigUtils.AppSetting<int>("ScrimpNet.Logging.Default.MaxFileSizeMb", MAX_FILE_SIZE_MB);
                }
            }
            /// <summary>
            /// Default: log
            ...
            public static string LogFileExtension
            {
                get
                {
                    return CoreConfig.Log.ReplaceTokens(ConfigUtils.AppSetting<string>("ScrimpNet.Logging.Default.FileExtension", FILE_EXTENSION));
                }
            }
            /// <summary>
            ...
            {
                return ConfigUtils.ReplaceKeyTokens(tokenizedString);
            }
        }
    }
}
