        ...
        public static class Log
        {
            ...
            public static LoggerLevels LoggerLevels
            {
                get
                {
                    ...
                    _loggerLevels  = ConfigManager.AppSetting<LoggerLevels>("ScrimpNet.Logging.LogLevels", LoggerLevels.Vital);
                    return _loggerLevels.Value;
                }
            }
            ...
            internal static string LastChanceLogFolder
            {
                get
                {
                    ...
                    string _lastChangeLogFolder = ConfigurationManager.AppSettings["ScrimpNet.Logging.LastChanceLogFolder"];
                    if (string.IsNullOrEmpty(_lastChangeLogFolder) == true || Directory.Exists(_lastChangeLogFolder) == false)
                    {                        
                        _lastChanceLogFolder = System.AppDomain.CurrentDomain.RelativeSearchPath;                         
                    }

                    return ReplaceTokens(_lastChangeLogFolder).Replace("{ts}", string.Format("{0:" + TimeStampFormat + "}", DateTime.Now));
                }
            }
            ...
            public static ILogPersister ActivePersister
            {
                get
                {
                    ...
                    string persisterClassName = ConfigManager.AppSetting<string>("ScrimpNet.Logging.Persister", "ScrimpNet.Diagnostics.InternalLogFilePersister,ScrimpNet.Core");
                    ...
                    try
                    {
                        ...
                        if (_activePersister == null)
                        {
                            ScrimpNet.Diagnostics.Log.LastChanceLog("Unable to create instance of {0} as ILogPersister", persisterClassName);
                            throw new DotNetConfig.ConfigurationErrorsException(TextUtils.StringFormat("Unable to create instance of {0} as ILogPersister", persisterClassName));
                        }
                    }
                    catch (Exception ex)
                    {
                        ScrimpNet.Diagnostics.Log.LastChanceLog(ex, "Unable to create instance of {0} as ILogPersister", persisterClassName);
                        throw new DotNetConfig.ConfigurationErrorsException(TextUtils.StringFormat("Unable to create instance of {0} as ILogPersister", persisterClassName), ex);
                    }
            ...
            public static string LogFolder
            {
             get
             {
                 ...
                 _logFolder =  ReplaceTokens(ConfigManager.AppSetting<string>("ScrimpNet.Logging.Default.LogFolder", Directory.GetCurrentDirectory()));
